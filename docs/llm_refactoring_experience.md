# 跨区重构经验：借蓝区强模型重构黄区算法代码

公司内网做算法，受两道墙夹着：一是原始代码不能出公司，合规红线；二是可用模型只有 glm-5.1、deepseek-v4-flash 这类较弱的内网模型，算法重构（提速、提精度、改架构）要的判断力它给不了，加上用的人多、常卡常中断，重度生成一旦断了难续上。一前一后：代码出不去，能用的模型又撑不起重构。

出路是把重构拆成两段，对准两道墙分别破。**内网模型只做"读和描述"**：读真实代码，提取项目的逻辑流程与架构，产出重构的 prompt 和规划步骤——这段是轻量任务，弱模型的判断力够用，卡了断了重试续上即可；产出的是抽象描述而非源码，能出公司，第一道墙让开。**外部模型做"按预定架构完成实现"**（kimi-3 / gpt-5.6 sol / claude-fable，经 codex / Claude Code 驱动）——这段要判断力和稳定输出，正是外部强模型的强项，第二道墙让开；而它从头到尾只碰 spec、碰不到内网真实代码和数据，合规也不破。内网模型读懂描述、外部模型转化执行，各做擅长的。

这套做法在 CryDet（婴儿啼哭检测）项目上完整跑过一遍，下面把这些经验和踩过的坑整理出来。意义在于：受合规约束、只有弱内网模型的团队，不必等内网追赶前沿，现在就能借外部强模型完成算法重构，经验也能沉淀成可携带的合约复用。文中的具体数字和案例都来自这次实战。

---

## 背景：黄区的局限

黄区是公司内网，有两个硬约束：原始代码不能出公司，可用模型是 glm-5.1、deepseek-v4-flash 等本土较新的模型。这类模型在日常问答、补全、改局部函数上够用，但在算法重构这类任务上明显吃力：

- **复杂架构设计**：给出"用Transformer做音频二分类，支持DDP多卡"这种需求，弱模型倾向于套视觉模型的常见做法（Conv1d patch embedding），方向直接偏掉
- **分布式调试**：DDP死锁这类问题需要追踪多个模块间的状态流转，弱模型只能盯着症状给表面修复
- **性能瓶颈分析**：要跨AudioReader/FeatureExtractor/Augmenter多个模块读代码识别热点，弱模型上下文和推理深度都不够

除了能力短板，黄区模型还有使用上的问题：用的人多，常常卡、慢、甚至中断。重度生成任务（一次产出几百行重构代码）一旦中断很难续上，体验和效率都差。

这三类任务恰好是重构的核心，又恰好是长输出、重生成的任务——弱模型做不动，拥堵中断更让它雪上加霜。黄区不单是缺判断力，还缺稳定的可用性。

---

## 为什么需要蓝区开发

蓝区是公司外网，能访问 kimi-3、gpt-5.6 sol、claude-fable 等当前最先进的外部模型，配合 codex、Claude Code 这类 harness 编码工具。把黄区搞不定的重构任务放到蓝区做，补的是模型的判断力，不是算力。

差异不是"快慢"，是能不能做对。以四类任务对照：
...
跨区不是"把代码搬出去跑"，而是"把黄区搞不定的问题，在蓝区用代表性项目做出来，把方法和合约带回去"。蓝区碰不到真实内网数据，所以用 CryDet 这类与内网问题同构的项目作为载体。
...
带回黄区的不是代码本身，是可复用的约束、模式和工作流（详见后文）。

---

## 跨区协同的优势

跨区不是简单"换个更强的模型"，而是把重构拆成两段，让黄区弱模型和蓝区强模型各做擅长的一段：

- **黄区段（提取 + 规划）**：内网大模型读真实代码，产出三样东西——项目的逻辑流程、当前架构、以及重构的 prompt 和规划步骤。这一段只读、只描述，不写实现，恰好是弱模型能做到的。
- **蓝区段（执行 + 实现）**：外部大模型拿到上述架构、流程和目标架构，直接做重构实现。这一段要的是判断力和跨模块推理，是强模型的强项。

这个分工带来几个具体优势：

1. **真实代码不出公司**。离开黄区的只是架构、流程、prompt 这些抽象描述，不含源码和数据，满足合规。外部模型从头到尾碰不到内网真实代码。
2. **双方短板都被绕开**。黄区弱模型不擅长复杂实现，这里只让它读懂和描述——读和描述是轻量任务，即便黄区模型拥堵、慢、中断，也容易重试续上；蓝区强模型碰不到真实数据，这里只让它按 spec 执行。各做擅长的，互不拖后腿。
3. **中间产物就是可携带的合约**。架构 + 流程 + 重构 prompt + 规划步骤，既是从黄区带到蓝区的输入，也是事后带回黄区复用的经验载体。同一份 spec 可以驱动不同的蓝区模型。
4. **强模型可随前沿替换**。spec 是模型无关的——蓝区今天用 kimi-3，明天换 gpt-5.6 sol 或 claude-fable，输入不用改。前沿更新时只换执行端，提取和规划段不受影响。
5. **经验是落地踩出来的，不是纸上谈兵**。spec 听起来对不够，要真在一个完整项目里跑过——哪些优化真提速了（如数据读取约 25×）、哪些反而把训练搞挂了（如 torch.compile 与 DDP 冲突），下面这些经验都来自实际做 CryDet 时遇到的，不是假设。
