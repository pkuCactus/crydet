...
| 工作量收益是多大？ | **重构工作量从预估 7-8 人月降至 1 人月**。收益分两层：① 范式自身贡献——**准入**（合规墙下使强模型可用）与**保真**（精度达到验收达标档 F1≥0.92）；② 强模型能力产出——后续模型演进成本下降（YAML 参数化，改档/改架构无需改代码）、训练可视化调优（TensorBoard loss/acc/lr/验证指标）、训练 epoch 4× 提速（2h→0.5h）。 |
...
开发在可调动强模型的小数据环境进行，验证在可触达真实数据的大数据环境进行，两端以代码与反馈衔接。"小/大数据"对本项目指 ML 训练数据规模；对非 ML 场景应理解为"低成本可调强模型环境 / 真实规模验证环境"的通用二分——等价形态如：本地小流量验证→生产流量压测、单元测试→端到端集成回归、小样本输入→全量数据回放。
...
> **重构工作量从预估 7-8 人月降至 1 人月**。须区分两层因果——**范式自身贡献**与**强模型能力产出**。

| 归因 | 收益 | 说明 |
|---|---|---|
| **范式·准入** | 在合规墙下使强模型可处理内部代码 | 跨区分工 + spec 出域使强模型"够得着"任务 |
| **范式·保真** | 精度达到验收达标档（F1≥0.92） | spec 保留 Fbank/能量过滤/mixup 等约束 + NOT 规则防错 + 通用基线测试兜底（三者贡献无法分离测量） |
| **强模型能力** | 后续模型演进成本下降、训练可视化调优、训练 epoch 4× 提速 | 强模型的生成与调优能力 |

落地指标（端到端精度与训练效率以黄区完整训练为准；模块级微基准仅用于开发期方向验证，不作为对外提速指标，因其易受缓存命中/单样本口径影响、不可外推端到端）：

| 指标 | 结果 | 测试条件 |
|---|---|---|
| 端到端精度 | **F1 达到验收达标档（≥0.92）**，达标（评估记录未随仓提交，为黄区训练实测） | 黄区完整数据（AudioSet+自采+购买） |
| 训练效率 | 单位 epoch **2h → 0.5h**，约 **4× 加速** | 黄区完整训练·单位 epoch |

精度达验收达标档——保真目标守住，训练效率、可维护性、演进成本下降与可视化调优为净收益。

### 跨区分工映射

| 阶段 | 环境 | 执行方 | 产出 |
|---|---|---|---|
| 提取规划 | 黄区（8 卡 V100/512G 内存） | 弱模型读真实代码 | spec：流程/架构/接口/约束 |
| 实现调优 | 蓝区（开发机） | 强模型按 spec 实现 | 约 1.3 万行代码 + CLAUDE.md |
| 大数据验证 | 黄区 | 黄区训练（人监控） | 达标档精度（F1≥0.92）、分布式/规模问题 |
...
| 6 | **文档跟着代码一起改** | 架构变更立刻同步项目级约束文件/docs（不限于 CLAUDE.md / AGENTS.md）。**原理**：强模型跨对话不保留上下文，延后补写成本陡增 |
...
>
> 上述判断有公开基准佐证：SWE-bench（公开多文件真实 bug 修复基准）上，头部模型解决率显著高于小参数模型；长上下文抓不稳中间信息（"lost in the middle"）、长输出到一半截断后接不回，亦是社区反复讨论的老问题。**注意：SWE-bench 度量的是 bug 修复，规格驱动从零生成无等价公开基准，此处为类比推断而非直接证据。** 这些现象落在重构任务上，即弱模型难以胜任、强模型能够胜任的几类工作。
...
- **每轮收敛判据**（各给可计量代理指标）：①指标达标率=本轮验收指标达标项数/总项数，阈值建议 ≥80%；②约束稳定度=本轮 CLAUDE.md/NOT 规则净增删条数，≤2 条为稳定；③反馈包规模=反馈条目数，连续轮次须单调递减。
- **退出条件**（命中任一即放弃本范式）：连续 2 轮反馈包不收敛；spec 提取轮耗时 ≥ 该轮强模型实现耗时的 50%（两端耗时试点中均可测）；弱模型盘点输出不可信、需大量人工修正。

---

## 后续规划

- **精度由达标到反超**：以 F1≥0.92 达标档为起点，将效率提升释放的机时投入调参与消融，目标在新架构上反超 baseline。
...
跨区协作并非一次性交接，而是循环——小数据开发、大数据验证、反馈修正规格，直至收敛。本次 CryDet 的收敛终点为：工作量由预估 7-8 人月降至 1 人月，精度达到验收达标档（F1≥0.92，范式保真贡献），后续模型演进成本下降（YAML 参数化就绪）、训练具备可视化调优闭环，训练效率与可维护性同步提升；约 1.3 万行代码连同收敛后的 spec + CLAUDE.md 一并带回黄区复用，后续同类任务可直接以此终点为起点。
