...
跨区协作不是"黄区提一次 spec、蓝区实现一次就完"。实际是个循环：蓝区跑出来的结果会反过来修正黄区的 spec，spec 再驱动下一轮蓝区实现，直到达标。

```
  ┌─── 黄区：弱模型读真实代码 → 产出 spec v1（架构/流程/接口/规划）
  │                          ↓
  │     蓝区：强模型按 spec 实现 → 跑训练/推理验证
  │                          ↓
  │     结果回流：代码、指标、报错（API 版本不匹配、优化破坏正确性…）
  │     打包成反馈带回黄区（带现象和结论，不带源码）
  │                          ↓
  │     黄区：弱模型读反馈 + 真实代码 → 更新 spec
  │     （补约束、调规划、改接口）
  │                          ↓
  │     蓝区：按更新后的 spec 再实现一轮
  │                          ↓
  └──── 收敛？指标达标、约束稳定 → 最终 spec + CLAUDE.md 合约带回黄区复用
```

这个循环的价值在于：错误和约束是在实战中一个个被撞出来、固化进 spec 的，不是黄区一次性"想全"。一次性的 spec 必然漏（漏掉版本兼容、漏掉分布式副作用、漏掉接口对不齐），循环才补得上。带回黄区的也不是某次实现的代码，而是收敛后的 spec 和 CLAUDE.md——下次同类任务直接复用，不用从零再走一遍。
