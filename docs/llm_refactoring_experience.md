...
3. **经验需要落地验证，不能只听理论**。spec 和优化方向听起来都对，但跑起来才知道——有的优化真有效，有的反而破坏正确性。具体提速倍数和翻车案例见下文 CryDet 的落地结果。

---

## 在 CryDet 上的落地结果

这次实践在 CryDet（婴儿啼哭检测）上跑出的实际结果：

- **训练效率**：数据读取加 LRU 内存缓存 + 线程局部 librosa，约 **25× 加速**；特征提取用 torch.compile + GPU-only 操作，约 **22× 加速**——两者把训练循环里 GPU 等 CPU 的空转消掉
