...
def setup_logger(
    ...
    if use_console:
        # Create console handler with rank-specific format
        handler = logging.StreamHandler(sys.stdout)
        handler.setLevel(effective_level)

        # Format includes rank info for distributed training
        if rank > 0:
...
def setup_file_logger(
    ...
    Returns:
        Configured logger instance.
    """
    logger = setup_logger(rank=rank, name=name, level=level)

    # Add file handler (only for rank 0 to avoid multiple writes)
    if rank == 0:
